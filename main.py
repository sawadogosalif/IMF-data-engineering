...
parser.add_argument(
    "--awscredentials", nargs=3, type=str,
...
def update_data(
        n_clicks, selected_columns,
        start_year, end_year, selected_countries
):
    if n_clicks is None:
        raise PreventUpdate
    ...
    if selected_countries:
        selected_data = selected_data[
            selected_data["pays"].isin(selected_countries)]

    # Apply the formatting function
    formatted_data = selected_data.sample(40).map(
        format_number).to_dict("records")

    # Extract the column names from the keys of
    # the first dictionary in formatted_data
